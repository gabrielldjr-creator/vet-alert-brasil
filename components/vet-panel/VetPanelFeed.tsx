  ...
  return (
    <section className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm font-semibold uppercase tracking-wide text-emerald-700">Feed de registros</p>
        ...
        <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-emerald-700 ring-1 ring-emerald-100">
          {alerts.length ? `${alerts.length} registros` : "Sem dados"}
        </span>
      </div>
      {alerts.length === 0 ? (
        <div className="flex min-h-[200px] flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 bg-slate-50 text-center text-slate-600">
          <p className="font-semibold text-slate-800">Nenhum registro corresponde aos filtros atuais</p>
          {totalAlerts > 0 ? (
            <p className="text-sm">Há {totalAlerts} registros carregados fora dos filtros selecionados.</p>
          ) : (
            <p className="text-sm">Ajuste os filtros ou registre novos sinais para atualizar o painel.</p>
          )}
        </div>
      ) : (
